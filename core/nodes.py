...
def dagTypes():
    """
    Returns a list of the currently loaded node types.
...
class DagNode(MutableMapping):
    ...
    def __setitem__(self, key, value):
        ...
        # auto-add attributes
        elif issubclass(type(value), Attribute):
    ...
    def __setitem__(self, key, value):
        if key in self.reserved:
            ...
            if key == 'name':
                ...
                if old_name != value:
                    if self.node:                        
                        self.node._attributes.pop(old_name)
                        self._name = value
                        setattr(self.node, self._name, self)

                        #update the network graph
                        if hasattr(self.node, '_graph'):
